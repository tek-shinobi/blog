...
Some thoughts and experiences when setting up AWS S3 and single-store for Grafana-Loki. Some context, using GLP stack (grafana, loki, promtail). Promtail collects logs from a Kafka stream and sends them to Loki for storage. It also indexes them by EC2 instance ids. 

## Why this post?
The official docs from Grafana are at times confusing. Also, I tried doing it in 3 ways, with different weirdness. Documenting here for future reference.
